...
This document provides a simple, deep, and 1:1 clear explanation of how to compile the cluaiz ecosystem and how to synchronize those compiled files into your local `.cluaiz` system folder (`~/.cluaiz` in installed mode or project-root `./.cluaiz` in development mode).

The philosophy is strict:

1. **Building** only compiles the code. It does NOT touch your system files.
2. **Syncing** is a manual step. You explicitly tell the system when to update your `.cluaiz` environment with your newly compiled code (In local development mode via `cargo run`, syncing targets your project-root `./.cluaiz` folder).
...
### 2. Build Interface Engines

Builds specific interface engines (ONNX / LLaMA) in isolation.

```bash
cargo run -p cluaiz-builder -- onnx --profile <debug|release>
cargo run -p cluaiz-builder -- llama --profile <debug|release>
```
...
### 5. Sync Interface Engines

Copies *only* the specific interface engine (ONNX / LLaMA) compiled artifacts into `./.cluaiz/engine/`.
