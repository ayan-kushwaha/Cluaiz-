...
Cancels an active chat stream mid-generation using its unique `stream_id`. This immediately interrupts the token generation loop and frees GPU/CPU resources without affecting other concurrent streams.
...
### 1. cURL
```bash
curl -X POST http://localhost:8080/v1/chat/cancel \
...
response = requests.post(
    "http://localhost:8080/v1/chat/cancel",
...
```javascript
const response = await fetch("http://localhost:8080/v1/chat/cancel", {
  method: "POST",
...
let res = client.post("http://localhost:8080/v1/chat/cancel")
