...
```bash
cluaiz run <model-identifier> [options]
```
...
### Examples:
```bash
cluaiz run bonsai:8b
cluaiz run gemma4:e2b --interactive false
```
...
1. **Vault Verification:** Verifies if the requested `<model-identifier>` is present in the local vault (`~/.cluaiz/models/chat/`).
