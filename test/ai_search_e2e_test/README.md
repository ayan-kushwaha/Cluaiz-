...
- **Purpose:** Full end-to-end test that verifies the complete AI ↔ `cluaiz-search` pipeline: a raw user prompt flows through the AI for query extraction, the `cluaiz-search` plugin performs a live web search, and the AI synthesizes a final answer from the results. Both the plugin's raw output and the AI's final response are captured in a single JSON file.
...
graph TD
    ...
    B -->|"Tight search query (3-8 words)"| C["Step 2: cluaiz-search plugin (/v1/execute/cluaiz-search/search)"]
...
- `src/main.rs`:
  ...
  - **Flow:** Engine health check → `extract_search_query()` → `run_search_plugin()` → `synthesize_answer()` → write JSON.
...
- `src/main.rs → extract_search_query()`:
  ...
  - **Why:** Ensures the plugin receives a tight, precise query instead of a full natural-language sentence.

- `src/main.rs → run_search_plugin()`:
...
- **Potential Failure Point:** plugin returns an unexpected JSON shape (e.g., `{ "result": "...json-string..." }` wrapper).
- **Recovery Logic:** `run_search_plugin()` attempts to parse the inner string as JSON; if that fails, wraps it as `{ "raw_result": "..." }` so the test never panics.
