...
async fn test_agnostic_sovereign_autonomy() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let target_manifest = roster.iter()
        .find(|m| (m.id.to_lowercase().contains("gemma-4") || m.name.to_lowercase().contains("gemma-4")) && m.local_path.is_some())
        .ok_or("❌ SOVEREIGN ERROR: Target model 'Gemma-4' not found")?;

    let target_name = &target_manifest.name;
    let target_path = std::path::PathBuf::from(target_manifest.local_path.as_ref().ok_or("❌ Manifest missing path")?);
