...
function Complete-Step {
    # Returns to start of line and replaces [ ] with [OK] in Green
    $out = "`r  " + $GREEN + '[OK]' + $NC
    Write-Host -NoNewline $out
    Write-Host ''
}
...
function Write-Success ([string]$msg) { 
    Write-Host ("`n  " + $GREEN + "[SUCCESS] " + $msg + $NC)
}
...
# --- Robust Download Engine (With Simple Blink Feel) ---
function Invoke-CluaizDownload ([string]$url, [string]$path, [string]$label) {
    ...
    while (-not $Success -and $RetryCount -lt $MaxRetries) {
        try {
            # We use curl directly; it provides its own progress if we don't use --silent
            # But for "Industrial" look, we keep it clean.
...
try {
    ...
    Invoke-CluaizDownload -url $CliBins.($Arch) -path $TargetCli -label ('Retrieving CLI (' + $Arch + ')')
    
    # 🚀 Zero-Copy Linkage
    $BinPath = Join-Path $HubPath 'bin'
    $BinLink = Join-Path $BinPath 'cluaiz.exe'
    Write-Step 'Linking CLI Gateway'
    if (Test-Path $BinLink) { Remove-Item $BinLink -Force }
    
    # Safe mklink invocation (Hardlink to avoid 17MB copy)
    $cmdArgs = '/c mklink /H "' + $BinLink + '" "' + $TargetCli + '"'
        ...
        foreach ($k in $KernelsToSync) {
            if ($KerBins.$k) {
                ...
                if ($KUrlRaw) {
                    ...
                    $KName = 'archer_' + $k + '.dll'
                    Invoke-CluaizDownload -url $KUrl -path (Join-Path $HubPath "interface-engines/kernels/$KName") -label ('Retrieving ' + $k + ' Kernel')
                }
            }
        }
    }
