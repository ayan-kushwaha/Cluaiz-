...
<h2 align="center">Cluaiz AI Inference Engine Rust Orchestrator for Local LLMs</h1>
...
cluaiz is **NOT** a new AI model, nor a new math kernel—it is an execution infrastructure that manages existing inference tools intelligently, ensuring they survive production constraints.
...
A core architectural pillar of cluaiz is the **cluaiz Expression Language (CEL)**. Instead of forcing developers to download heavy SDKs or manage complex state externally, cluaiz provides a deterministic execution language baked directly into the engine.

<p align="center">
  <img src="assets/devhub-img.png" width="100%" alt="cluaiz DevHub API Interface">
</p>

### **The Direct CEL API (No SDK Required)**

Most standard AI engines expose basic REST endpoints for text generation. cluaiz exposes a dynamic CEL compilation endpoint. You can send raw CEL scripts directly to the engine via HTTP.
...
Traditional tools rely on executing LLM-generated code in insecure local terminals or external containers. cluaiz takes a fundamentally different approach:

1. **Manifest-Driven Extensions:** When you download a plugin or skill from the cluaiz Hub, it includes a `manifest.yaml`. This acts as a strict execution contract.
...
- **Reproducible Binary Routing**: Ensuring consistent inference results across platforms via cluaizdbNA.
...
### **The cluaizdbNA standard**

A decoupled, three-tier modular design that ensures zero-drift between the CLI, the Engine, and the bare-metal Drivers.

```mermaid
graph TD
    A[Interface: CLI/SDK] -- "Optimized Signaling" --> B["cluaiz Engine"]
    B -- "cluaizdbNA Manifest" --> C[Model Registry]
...
# 2. Build the entire ecosystem (Core + All Native Drivers) using the cluaiz Builder
$ cargo run -p cluaiz-builder -- all --profile release
...
*(Aliases: `cluaiz api`, `cluaiz server`)*
