...
/// 🏛️ Cluaize Sovereign SDK: The Universal Handshake
/// Returns the current machine's sovereign identity as a C-string.
/// 
/// # Safety
/// The caller must free the returned string using `cluaize_free_string`.
#[no_mangle]
pub extern "C" fn cluaize_get_identity() -> *mut c_char {
    let governor = HardwareGovernor::start();
    let identity = format!("Cluaize-OS-Sovereign-v1:Ready={}", governor.is_ready());
    
    CString::new(identity).unwrap().into_raw()
}

/// ⚖️ Cluaize Sovereign SDK: VRAM Inquiry
/// Returns the total available VRAM in GB.
#[no_mangle]
pub extern "C" fn cluaize_get_total_vram() -> f64 {
...
/// 🛡️ Cluaize Sovereign SDK: Memory Release
/// Frees strings allocated by the SDK to prevent memory leaks in the host language.
#[no_mangle]
pub extern "C" fn cluaize_free_string(s: *mut c_char) {
    if s.is_null() { return; }
    unsafe {
