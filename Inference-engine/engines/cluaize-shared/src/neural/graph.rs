...
impl NeuralGraph {
    ...
    fn resolve_graph_path() -> PathBuf {
        ...
        for _ in 0..10 {
            // Check if we are in the workspace root by looking for 'cluaize-engine' or 'thing.ai.nurale.md'
            if current.join("cluaize-engine").exists() || current.join("thing.ai.nurale.md").exists() {
