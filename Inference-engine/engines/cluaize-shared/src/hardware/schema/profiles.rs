...
pub struct SovereignContext {
    pub cluaize_root: String,
}

#[derive(Debug, Clone, Serialize, Deserialize, Default, Archive, RkyvSerialize, RkyvDeserialize)]
#[archive(check_bytes)]
pub struct SovereignBrain {
    #[serde(default = "default_cluaizd_connect_ffi")]
    pub cluaizd_connect_ffi: String,
}

fn default_cluaizd_connect_ffi() -> String {
    "off".to_string()
}

impl SovereignBrain {
    pub fn is_enabled(&self) -> bool {
        self.cluaizd_connect_ffi != "off" && !self.cluaizd_connect_ffi.is_empty()
    }

    pub fn is_pure_brain(&self) -> bool {
        self.cluaizd_connect_ffi == "only"
    }

    pub fn is_local(&self) -> bool {
        self.cluaizd_connect_ffi == "on" || self.is_pure_brain()
    }
}
...
/// 🧬 Cluaize Bridge Alias: CluaizeProfile is the canonical name for SovereignProfile.
/// Kept for backward compat during naming migration.
pub type CluaizeProfile = SovereignProfile;
