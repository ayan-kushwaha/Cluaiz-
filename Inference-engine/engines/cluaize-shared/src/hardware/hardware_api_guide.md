...
Ye document ek "API Guide" hai jo batata hai ki Cluaize Backend Engine (`archer-shared`) ko Frontend, CLI, ya kisi doosre software se kaise call kiya jayega. **(No `cargo test` required in production!)**
...
**Kaise Call Karein (In Rust CLI/Frontend):**
```rust
use cluaize_shared::hardware::system_performance::SystemPerformanceLive;
...
1. **`cluaize hardware`** -> CLI `system_control.rs` ko call karegi aur hardware ki poori kundli screen par print kar degi.
2. **`cluaize monitor`** -> CLI `system_performance_live.rs` ko call karegi aur ek **Live Hacker-Style TUI** (Progress bars, changing colors) khol degi jo seedha Atomic memory se data padhega.
