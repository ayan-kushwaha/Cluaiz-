...
impl HardwareOrchestrator {
    ...
    fn probe_context() -> SovereignContext {
        ...
        SovereignContext {
            cluaize_root: root_path.to_string_lossy().to_string(),
        }
    }
    ...
    fn probe_storage() -> Vec<StorageSubsystem> {
        ...
        // 🏠 Resolve the primary home of Cluaize-OS
        let cluaize_home = dirs::config_dir().unwrap_or_default().join("Cluaize");
        let cluaize_home_str = cluaize_home.to_string_lossy();
        ...
        for disk in &disks {
            ...
            // 🎯 Match: Is this disk hosting the Cluaize kernel?
            let is_primary = cluaize_home_str.contains(&path);
