...
use engine_lmdb::ffi::{cluaizd_ffi_read_neuron, cluaizd_ffi_free_neuron_payload, cluaizd_ffi_execute_parameterized, CluaizdFfiNeuron};
use engine_lmdb::env::LmdbEnv;
use std::ffi::c_void;
use std::sync::OnceLock;

pub static GLOBAL_LMDB_ENV: OnceLock<LmdbEnv> = OnceLock::new();

pub struct TensorTransducer;

impl TensorTransducer {
    pub fn boot_environment() {
        let db_path = dirs::home_dir().unwrap_or_default().join(".cluaiz").join("cluaizd");
        if !db_path.exists() {
            let _ = std::fs::create_dir_all(&db_path);
        }
        // Open with 1GB capacity for now
        match LmdbEnv::open(&db_path, 1024 * 1024 * 1024) {
            Ok(env) => {
                let _ = GLOBAL_LMDB_ENV.set(env);
                tracing::info!("🧠 LMDB Environment fully booted for Zero-Latency FFI at {:?}", db_path);
            }
            Err(e) => {
                tracing::error!("Failed to boot LMDB Environment: {:?}", e);
            }
        }
    }
    ...
    pub fn inject_context(memory_key: &str) -> Option<Vec<u8>> {
        ...
        // 2. Transduce: Call the Database FFI
        let env = GLOBAL_LMDB_ENV.get()?;
        ...
        if result != 0 || out_neuron.payload_ptr.is_null() {
            tracing::debug!("FFI Brain lookup failed or not initialized for key: {}", memory_key);
            return None;
        }
        ...
        let payload = unsafe {
            ...
            cluaizd_ffi_free_neuron_payload(out_neuron.payload_ptr as *mut u8, out_neuron.payload_len, out_neuron.payload_len);
            
            vec_data
        };

        tracing::info!("🧠 Successfully injected Neural Context via Zero-Latency FFI: {} bytes", payload.len());
        Some(payload)
    }
    ...
    pub fn save_context(memory_id: &str, payload: &str, vector: [f32; 16]) -> Result<(), String> {
        ...
        let env = GLOBAL_LMDB_ENV.get().ok_or("LMDB Environment not booted")?;
        let env_ptr = env as *const LmdbEnv as *mut c_void;

        // Create the parameterised CDQL Query Shell
        let query = format!("insert into Context(id: \"{}\", payload: \"{}\", vector: ?)\0", memory_id, payload);
        ...
        if result == 0 {
            tracing::info!("💾 Successfully saved Memory Context via Parameterized Zero-Latency FFI.");
            Ok(())
        } else {
            let err_msg = format!("FFI Write Failed with status code: {}", result);
