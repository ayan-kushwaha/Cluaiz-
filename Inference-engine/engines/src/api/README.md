# 📡 Cluaize Telemetry Bridge (`engines/src/api/`)
...
The `engines/src/api/` directory is **NOT** the external HTTP gateway (that lives in the top-level `api/` crate). Instead, this is the internal **Cluaize Telemetry Bridge (Archer Server)**. 
...
graph TD
    ...
    TCP -->|"GET /dashboard"| HTML["Cluaize_Dashboard.html"]
