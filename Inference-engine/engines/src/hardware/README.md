...
The `hardware/` module acts as the physical layer interface for the Cluaize Inference Engine. Traditional LLM execution environments (like `llama.cpp` or Ollama) often require users to manually specify execution flags: how many threads to use, how many layers to offload to the GPU, or whether AVX2 is available. 
...
- **The Core Logic:** Re-exports the `SiliconTruth` structures defined in the `cluaize-shared` crate and implements the `HardwareDetector` bootloader.
...
### 3. `models_runner.rs`
- **The Core Logic:** The execution orchestrator. Takes the `SiliconTruth` generated by the `System Control Manager` and decides which backend runner (`Cluaize`, `Llama`, or `Candle`) is mathematically optimal for the user's specific silicon.
