...
pub async fn install_skill(
    ...
    tokio::spawn(async move {
        let _ = engines::neural_foundry::registry::SkillRegistry::install_skill(&skill_name).await;
    });
...
pub async fn clear_cache(State(_state): State<Arc<AppState>>) -> Json<Value> {
    match engines::neural_foundry::registry::SkillRegistry::clear_skills_cache(None, true, true) {
