...
pub async fn list_skills(State(_state): State<Arc<AppState>>) -> Json<Value> {
    match ToolHubInstaller::list_installed_components("skill") {
...
pub async fn install_skill(
    ...
    tokio::spawn(async move {
        let _ = ToolHubInstaller::install_component("skill", &skill_name).await;
    });
...
pub async fn list_cache(State(_state): State<Arc<AppState>>) -> Json<Value> {
    match ToolHubInstaller::list_component_cache("skill") {
...
pub async fn clear_cache(State(_state): State<Arc<AppState>>) -> Json<Value> {
    match ToolHubInstaller::clear_component_cache("skill", None, true, true) {
...
pub async fn remove_skill(
    ...
    let skill_name = payload.skill_name.clone();
    match ToolHubInstaller::remove_component("skill", &skill_name).await {
